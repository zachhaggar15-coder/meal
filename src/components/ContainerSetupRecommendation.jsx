...
export default function ContainerSetupRecommendation({
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(entries => {
      ...
      track.containerRecommendationViewed({
        ...
        cta_location: 'plan_container_recommendation',
      });
      observer.disconnect();
    }, { threshold: [0.35] });
  ...
  return (
    <section ref={sectionRef} className={`container-setup-recommendation ${className}`.trim()} aria-labelledby={`${sourcePage}-container-setup-heading`}>
